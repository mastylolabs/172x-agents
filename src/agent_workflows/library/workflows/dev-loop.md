...
## Inputs
The user's task, repository instructions, relevant project context, and the matching local `.172x/contexts.toml` activation entry. No change-request number is an input: the coordinator discovers or creates every change request needed by the loop.
...
## Flow
1. Read the matching local `.172x/contexts.toml` activation entry and validate the active language and expected gate tools before any work. `brief-author` converts the task and supplied context into one identified focused brief with source/evidence state, acceptance criteria, non-goals, assumptions, unresolved decisions, risks, and the active engineering gate.
...
## Human gates
The developer explicitly activates the current project locally before using the configured dev-loop Git/GitHub actions. A clean generic non-main branch is handled only after the active context's gate passes. The coordinator must still stop for user direction when the workspace is dirty, `main` cannot be updated safely, a `Q` requires a product decision, an external command cannot run, or the two-review-trip limit is reached.
